...
### 🟢 BIG SESSION 2026-06-17 — Tier 1/1.5 shipped + Site Audit started (all pushed)
User direction: tackle Tier 1 (report quality, whole-site crawl), Tier 1.5 (suppress), Tier 2 (baseline). **Dropped by user this session:** white-label PDF + per-client workspace (see §1). Then rewrite PRODUCT_VISION from scratch (after these). Shipped, each verified on americas.land + tsc/lint/build clean:
...
- **Phase 3 — Site Audit (crawl→select→audit→rollup), IN PROGRESS:** design = combined flow, **tree-select** (pick pages individually or by parent subtree to skip template-dupes — user's req), audit **every selected** page, report = rollup + worst-pages + site-wide finding rollup. `f39ebef` **3a** (site_audits table + scans.site_audit_id + urlsInSubtree helper) · `9e67e0e` **3b** (`/site-audit/new` + `/site-audit/[id]` branch on status; discovery reuses crawler; `SiteTreeSelect` checkbox tree; top-nav "Site Audit" group). **TODO: 3c** orchestration (fan out a scan per selected URL tagged siteAuditId + progress) · **3d** aggregated site report. (Tasks 10/11.)
...
- **Scoring-fidelity follow-ups (Tier 3, PARKED — added this session):** (1) **isolate Lighthouse from scan CPU contention** — during a scan benchmarkIndex drops 2762 vs ~4800 isolated → perf 48 vs 72-73; give LH the host to itself. (2) **median-of-3 Lighthouse runs** per form factor — cold run1=52 vs run2/3=72-73; report median (LHCI best practice), optionally show min-max range. Diagnosed via isolated 3× run on walleyproperties; CLS 0.298 is a real site issue (constant across runs). **Location (RO user vs TX server) is NOT a factor — simulate throttling uses fixed RTT/throughput.**
